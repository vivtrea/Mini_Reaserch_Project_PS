...
$H_0$: resistant and susceptible groups have the same median, H_1: resistant and susceptible groups have not the same median

```{r}
wilcox.test(group_r, group_s, alternative = "two.sided")
```
W is large and p-value is greater than 0.05 so resistant and susceptible groups have the same distribution and means

We decided to use permutations test to check whether means are equal:

$H_0: \mu_R = \mu_S \quad H_1: \mu_R \neq \mu_S$
test statistic = $\mu_R - \mu_S$
